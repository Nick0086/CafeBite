...
const CHECKPOINT_PATH = path.join(__dirname, 'seed-india-checkpoint.json');
...
const load = async () => {
  ...
  if (cp.lastDate !== today()) { cp.lastDate = today(); }
};
...
const main = async () => {
  ...
  if (clean) {
    ...
    cp = { cscCountries: null, indiaStates: null, stateMap: {}, statesFetched: false, citiesFetched: [], lastDate: today() };
    await save();
    console.log('Tables cleaned.');
  }

  if (!cp.cscCountries) {
    console.log('Fetching all countries from CSC API...');
  ...
  console.log('Inserting all currencies...');
  ...
  if (!cp.statesFetched) {
    console.log('Fetching India states...');
    cp.indiaStates = await csc('/countries/IN/states');
    if (!cp.indiaStates || cp.indiaStates.length === 0) {
      console.log('Failed to fetch India states.');
      await db.end();
      process.exit(1);
    }
    await save();
    console.log(`  Got ${cp.indiaStates.length} states`);
  }

  console.log('Inserting India states...');
  const indiaCountry = cp.cscCountries.find(c => c.iso2 === 'IN');
  for (const s of cp.indiaStates) {
    await db.execute(
      `INSERT IGNORE INTO ucmt_tbl_state_master (state, countryid) VALUES (?, ?)`,
      [s.name.slice(0, 100), indiaCountry.id]
    );
  }
  ...
  const stateMap = {};
  for (const row of stateRows) stateMap[row.state] = row.id;
  for (const s of cp.indiaStates) {
    cp.stateMap[s.iso2] = stateMap[s.name.slice(0, 100)];
  }
  cp.statesFetched = true;
  await save();
  console.log(`  Inserted ${cp.indiaStates.length} states`);

  console.log('Fetching and inserting India cities...');
  for (const s of cp.indiaStates) {
    const key = s.iso2;
    ...
    const cities = await csc(`/countries/IN/states/${s.iso2}/cities`);
    ...
    cp.citiesFetched.push(key);
    await save();
    console.log(`  ${s.name}: ${cities?.length || 0} cities`);
  }
  ...
  console.log(`States:     ${st[0].c} (India only)`);
  console.log(`Cities:     ${ci[0].c} (India only)`);

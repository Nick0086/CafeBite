...
export default function ClientSupportIndex() {
    ...
    const [selectedTab, setSelectedTab] = useState(subPath || 'dashboard');

    useEffect(() => {
        if (subPath && subPath !== selectedTab) {
                ...
                <Tabs value={selectedTab} className="border-none w-full" onValueChange={handleTabChange}>
                    <TabsList className="flex flex-wrap w-full border-b border-gray-300">
                        <TabsTrigger
                            value="dashboard"
                            variant="team"
                            className="text-blue-500 border-blue-500 data-[state=active]:bg-blue-200 data-[state=active]:text-blue-700"
                        >
                            DashBoard
                        </TabsTrigger>
                        <TabsTrigger
                            value="feedback"
                            variant="team"
                            className="text-red-500 border-red-500 data-[state=active]:bg-red-200 data-[state=active]:text-red-700"
                        >
                            Feedback
                        </TabsTrigger>
                    </TabsList>

...
const MenuItemForm = memo(({ open, onHide, isEdit, selectedRow, isDireact }) => {
    ...
    useEffect(() => {
        if (categoryError) {
            toast.error(`Error fetching Category: ${categoryError?.err?.message}`);
        }
    }, [categoryError]);
    ...
    const createMenuItemMutation = useMutation({
        ...
        onSuccess: (res) => {
            ...
            toastSuccess(res?.message || `Menu Items : ${menuItemName} added successfully`);
            handleModalClose();
        },
        onError: (error) => {
            toastError(`Error adding Menu Items: ${error?.err?.error}`);
        }
    });
    const updateMenuItemMutation = useMutation({
        ...
        onSuccess: (res) => {
            queryClient.invalidateQueries(menuQueryKeyLoopUp['item']);
            toastSuccess(res?.message || `Menu Items : ${categoryName} updated successfully`);
            handleModalClose();
        },
        onError: (error) => {
            toastError(`Error updating Menu Items: ${error?.err?.error}`);
        }
    });
        ...
        <Dialog open={open} onOpenChange={handleModalClose}>
            <DialogContent className='min-w-[40%]' >
                {open && (
                    <>
                        <DialogHeader>
                            <DialogTitle>{isEdit ? `Edit Menu Items` : `Create New Menu Items`}</DialogTitle>
                        </DialogHeader>
